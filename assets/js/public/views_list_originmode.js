...
function getViewsOriginModel() {
    ...
    var source_id = localStorage.getItem('trackview_source_id'),
        medium_id = localStorage.getItem('trackview_medium_id');

    if(source_id == false || medium_id == false){
        return false;
    }
    ...
    var stringQuery = "SELECT count(u1.id) as conta, CONCAT(q1.value1, '_', q2.value2) as value FROM events AS u1 INNER JOIN (SELECT event_id, value as value1 FROM properties WHERE ref_propertie = "+source_id+") as q1 ON u1.id = q1.event_id INNER JOIN (SELECT event_id, value as value2 FROM properties WHERE ref_propertie = "+medium_id+") as q2 ON u1.id = q2.event_id WHERE ref_event = 1 AND ("+filterDate+") GROUP BY value ORDER BY conta DESC";
